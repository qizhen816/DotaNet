...
DataFile = ["data_set0816.txt"] #0724_20
LabelFile = ["labe_set0816.txt"] #0724_20
topList = 'TeamTop816.txt' #0724

DataFile_new = ["data_set190813.txt"] #0724_20 0820
LabelFile_new  = ["label_set190813.txt"] #0724_20 0820
topList_new  = 'TeamTop190813.txt' #0724 0820
...
class RawData(object):
    ...
    def storeData(self,filename,all,israndom = True):
        ...
        if len(all) == 2:
            ...
            data_set, label_set,rank = all
            toZip = list(zip(data_set, label_set,rank))
            if israndom == True:
                random.shuffle(toZip)
            data_, label_ ,ranks= map(list, zip(*toZip))
            Data = np.asarray(data_, dtype=float)
            Lable = np.asarray(label_, dtype=float)
    ...
    def check_zeros(self,all,maxZeros = 150):
        ...
        for err in error:
            if err != -1:
                ...
                label = np.delete(label, err - p + 1, 0)
                if  len(all) == 3:
                    ranks = np.delete(ranks, err - p + 1, 0)
            p += 1
        print('After:',len(data))
        print('完成')
        if len(all) == 2:
    ...
    def make_double(self,all,type):
        ...
        dataNew = []
        labelNew = []
        for i in range(trainPart, trainPart+crossPart):
            dataTmp = [0] * len(data[0])
            dataTmp[0:datalen] = list(data)[i][datalen:datalen * 2]
            dataTmp[datalen:datalen * 2] = list(data)[i][0:datalen]
            dataTmp[datalen * 2:datalen * 2 + 3] = list(data)[i][datalen * 2 + 3:datalen * 2 + 6]
            ...
            labelNew.append(1 - float(label[i]))
            dataNew.append(dataTmp)
        dataNew += list(data)[trainPart:trainPart+crossPart]
        labelNew += list(label)[trainPart:trainPart+crossPart]
        toZip = list(zip(dataNew[:], labelNew[:]))
        random.shuffle(toZip)
        crossData, crossLabel = map(list, zip(*toZip))

        Data += list(crossData)
        Label += list(crossLabel)
        print('交换之后，训练集+验证集长度：',len(Data))

        Data += list(data)[trainPart+crossPart:]
        Label += list(label)[trainPart+crossPart:]
    ...
    def make_rnn(self,all,alpha = 5,isgamma = True,teamRank = False):
        ...
        for items in data:
            ...
            p = 0
            for team in teams:
                if int(team[0]) == int(items[-6]):
                    teamID1 = 21 - p
                if int(team[0]) == int(items[-3]):
                    teamID2 = 21 - p
                if teamID1 != -1 and teamID2!= -1:
                    break
                p += 1
            ranks.append([21-teamID1,21-teamID2])
            if teamRank == True:
    ...
    def pre_RNN(self,old = True):
        ...
        All = self.make_rnn(All,alpha = 10,isgamma = True,teamRank = False) #ABAB...AB33 #顺序正常
       # All = self.check_double(All)
        #self.check(All)
...
def go():
    ...
    filename = 'DotaTMzScore180814.set'
    # filename = 'DotaTM.set'
    ...
    Obj.storeData(filename,All,israndom=False)

...
class DotaModel(object):
    def __init__(self, sess,train_length,layer_units,net, drop_out_rate,n_step,
    ...
    def train(self,config):
        ...
        for epoch in range(0,config.epoch):
            ...
            for idx in range(0,batch_idxs_train):
                feed_dict = self.feed_data(train_X,train_Y,idx,self.drop_out_rate,learning_rate)
            ...
            for idx in range(0, batch_idxs_test):
                ...
                test_acc_,summary_str_c = self.sess.run([self.accuracy,self.ab_sum],feed_dict=feed_dict)
                self.test_writer.add_summary(summary_str_c, counter_)
    ...
    def run_rnn(self,X,isOnly = True,n_hidden = 0,reuse = False,scopename = "RNN_layers"):
        with tf.variable_scope(scopename) as scope:
            ...
            #lstm_cell = normlized_lstm(n_hidden)
            #lstm_cell = rnn.BasicLSTMCell(n_hidden)
            #lstm_cell = tf.nn.rnn_cell.BasicRNNCell(n_hidden)
            lstm_cell = tf.nn.rnn_cell.GRUCell(n_hidden)
            if self.Drop_out == True:
                lstm_cell = tf.nn.rnn_cell.DropoutWrapper\
                    (lstm_cell, output_keep_prob=self.keep_prob,input_keep_prob=self.keep_prob)  # drop_out

            init_state = lstm_cell.zero_state(self.batch_size, dtype=tf.float32)
            outputs, states = tf.nn.dynamic_rnn(lstm_cell, X_RNN, initial_state=init_state, dtype=tf.float32,time_major= False)
            output = tf.unstack(tf.transpose(outputs, [1, 0, 2]))[-1]

            #return tf.reduce_mean(outputs,axis=1)
            rnn_out = tf.matmul(output,tf.get_variable('OutRNN_w',[n_hidden, n_hidden//2],
                    initializer=tf.truncated_normal_initializer(stddev=0.0000001))) \
                      + tf.get_variable('OutRNN_b', [n_hidden//2], initializer=tf.constant_initializer(0.0))
    ...
    def run_mix(self, X, isOnly = True,reuse = False,scopename = "Mix_layers"):
        with tf.variable_scope(scopename) as scope:
            ...
            if self.mix_cnn == True:
                ...
                conv1 = conv2d_rnn(x,input_dim = 1,
                                   output_dim=n_conv1,k_h=1,k_w=5,d_h=1,d_w=5,stddev = 0.0000001,name='conv1',reuse= reuse)

                tf.summary.histogram("activations1", conv1)

                conv1 = (self.d_bn1(conv1))
                #conv1 = tf.nn.relu(conv1)
                conv2 = conv2d_rnn(conv1,input_dim = n_conv1,
                                   output_dim=n_conv2,k_h=1,k_w=4,d_h=1,d_w=4,stddev = 0.000001,name='conv2',reuse= reuse)
                conv2 = (self.d_bn2(conv2))
                #conv2 = tf.nn.relu(conv2)

                # tf.summary.histogram("activations2", conv2)

                conv3 = conv2d_rnn(conv2,input_dim = n_conv2,
                                   output_dim=n_conv3,k_h=1,k_w=2,d_h=1,d_w=2,stddev = 0.000001,name='conv3',reuse= reuse)
                #tf.summary.histogram("activations3", conv3)

                conv3 = self.d_bn3(conv3)

                #conv3 = lrelu(self.d_bn3(conv3)) tf.nn.relu
                ...
                X_results = tf.reshape(X_results, [5,n_steps,1]) #？ 5->-1?
                ...
                X_RNN = tf.matmul(X_RNN, tf.get_variable('InRNN_w',[n_data, n_hidden],
                         initializer=tf.truncated_normal_initializer(stddev=0.000001))) \
            ...
            if reuse == True:
                ...
                sn = 'b'
            #如果共用CNN的话下面的要减少缩进
            rnn_out = self.run_rnn(X_RNN,isOnly=False,n_hidden = n_hidden+1,reuse = False,scopename = sn)
        ...
        return rnn_out
        # return   tf.nn.dropout(tf.reshape(X_RNN,[5,n_steps*(n_hidden+1)]),keep_prob=self.keep_prob)
    ...
    def run_rnnx2(self,X,reuse = False,scopename = "2RNN_layers"):
        with tf.variable_scope(scopename) as scope:
            if reuse:
                scope.reuse_variables()
            n_data = 41
            X1 = tf.reshape(X[:,0:self.n_step*n_data],[-1,self.n_step,n_data])
            X2 = tf.reshape(X[:,self.n_step*n_data:self.n_step*n_data*2],[-1,self.n_step,n_data])

            # A = tf.constant(23.0,shape = [self.batch_size,1]) - tf.reshape(X[:,-6],[self.batch_size,1])
            # B = tf.constant(23.0,shape = [self.batch_size,1]) - tf.reshape(X[:,-3],[self.batch_size,1])
            # A_B = tf.concat([A-B,B-A],1)
            ...
            # out = tf.nn.dropout(out,keep_prob=self.keep_prob)
            #tf.truncated_normal_initializer(stddev=0.00001)))\
            #math.sqrt(2.0 / float(out_w)))))\ 1.0 / math.sqrt(float(out_w))))) \ tf.contrib.layers.xavier_initializer())) \
            # a_out = (tf.reshape(out[:,0],[self.batch_size,1]) * A)
            # b_out = (tf.reshape(out[:,1],[self.batch_size,1]) * A)
            # out = tf.concat([a_out,b_out],1)
            #out = A_B + out
